...
const ContactPage = () => {
  ...
  return (
    <div className="relative bg-cover bg-center">
      ...
      <div className="relative z-10  mx-auto px-4 sm:px-6 lg:px-8 py-16 md:py-24 flex flex-col lg:flex-row justify-center items-start lg:space-x-12">

        {/* Slider bên trái */}
        <div className="bg-gray-300 backdrop-blur-sm rounded-xl p-6 md:p-8 lg:p-10 w-full lg:w-3/4 max-w-3xl mb-12 lg:mb-0 shadow-xl">
          <Slider {...sliderSettings}>
            {slides.map((slide) => (
              <div key={slide.id} onClick={() => router.push(slide.link)} className="w-full cursor-pointer">
                <img src={slide.img} alt={`Slide ${slide.id}`} className="w-full h-118 object-cover rounded-xl" />
              </div>
            ))}
          </Slider>
        </div>
